...
export function ProjectDrive({ stories, story, onChanged, saveOnly = false }: { stories: Story[]; story?: Story; onChanged: () => void; saveOnly?: boolean }) {
  ...
  useEffect(() => {
    if (saveOnly) return;
    const closeForHours = () => setOpen(false);
    window.addEventListener('orbit-school-closed', closeForHours);
    return () => window.removeEventListener('orbit-school-closed', closeForHours);
  }, [saveOnly]);
  ...
  return <div className="project-drive">
    ...
    {open && createPortal(<div className="project-drive-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy && shouldDismissProjectDrive('BACKDROP')) setOpen(false); }}><section ref={dialog} tabIndex={-1} className="project-drive-dialog" role="dialog" aria-modal="true" aria-labelledby="drive-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      {busy && busy !== 'OPEN' && <LoadingStatus label={busy === 'FINISH' ? 'Packing your session…' : 'Saving your story…'} detail={progress || 'Preparing and checking the saved files. Keep this tab open.'} />}
      ...
      {website && !saveOnly && <section className="project-drive-finish"><div><h3>Taking Orbit off the grid?</h3><p>Tools load as you use them online. Pack the full toolkit only when you need every room without Wi-Fi. Your stories still need their own save.</p></div><a href={`${import.meta.env.VITE_READER_BASE || '/'}offline.html`} target="_blank" rel="noopener noreferrer">Prepare for offline use</a></section>}
