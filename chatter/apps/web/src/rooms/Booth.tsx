...
export function Booth({ stories, me, onChanged }: { stories: Story[]; me?: User; onChanged: () => void }) {
  ...
  return <section className="view on newsroom-room booth-room">
    <header className="newsroom-hero"><div className="newsroom-hero-icon"><RoomIcon kind="microphone" /></div><div><span className="newsroom-eyebrow">BOOTH</span><h1>Voice recording</h1><p>Check the microphone, rehearse, record, and choose a take.</p></div><label className="booth-story-picker">Story<select aria-label="Recording story" disabled={active || busy || !!pending} value={story.id} onChange={(event) => navigate(`/booth/${event.target.value}`)}>{stories.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label></header>
    ...
    {(busy || recorder.phase === 'SAVING' || recorder.phase === 'ASKING') && <LoadingStatus label={recorder.phase === 'ASKING' ? 'Connecting to your microphone…' : 'Preparing and saving your recording…'} detail={recorder.phase === 'ASKING' ? 'Allow microphone access in your browser when asked.' : 'Keep this tab open until the recording is saved.'} />}
