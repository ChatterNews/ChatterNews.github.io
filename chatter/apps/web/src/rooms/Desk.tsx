...
import { advanceStoryWorkflow, countWords, prosePlainText, readTimeSec, recordRole, saveDeliverable, storyAngleChecks, storyPath, type CrewTask, type ProseNode, type Story, type StoryReview, type User } from '@chatter/shared';
...
export function Desk({ stories, me, onChanged }: { stories: Story[]; me?: User; onChanged: () => void }) {
  const { storyId } = useParams(); const navigate = useNavigate(); const store = useStore();
  ...
  const save = useCallback(async (): Promise<boolean> => {
    if (!story || !editor || story.status === 'DONE' || saveState === 'SAVED') return true;
  ...
  }, [story, editor, saveState, store, me, onChanged]);
  ...
  async function handoff() {
    if (!story || !editor || busy) return; setBusy(true); setNotice(undefined);
    try {
  ...
  function applyShape(shape: DeskShape) { if (!editor || !story) return; if (stats.words && !window.confirm('Switch story routes? Your current draft will be replaced after the next save.')) return; editor.commands.setContent(deskShapeTemplate(shape)); setSideTab('CHECK'); setSaveState('DIRTY'); }
  function insertQuote(source: NonNullable<Story['brief']>['sources'][number]) { if (!editor || !source.quotes.trim()) return; editor.chain().focus().insertContent(sourceQuoteContent(source)).run(); }
  ...
  return <section className={`view on newsroom-room desk-room ${focus ? 'desk-focus' : ''}`}>
    <header className="newsroom-hero desk-hero"><div className="newsroom-hero-icon">✎</div><div><span className="newsroom-eyebrow">DESK</span><h1>Writing and editing</h1><p>Draft the words, use the project notes, and prepare the handoff.</p></div><label className="desk-story-picker">Story<select aria-label="Writing story" value={story.id} onChange={(event) => void changeStory(event.target.value)}>{stories.filter((item) => item.status !== 'DONE').map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label></header>
    ...
    <div className="desk-shell">
      {!focus && <aside className="desk-outline"><DeskRoutePicker selected={activeShape} recommended={recommendedShape} disabled={false} onChoose={applyShape} /></aside>}

      <main className="desk-paper"><div className="desk-toolbar" role="toolbar" aria-label="Writing tools">
        <button aria-label="Undo" disabled={!editor?.can().undo()} onClick={() => editor?.chain().focus().undo().run()}>↶</button><button aria-label="Redo" disabled={!editor?.can().redo()} onClick={() => editor?.chain().focus().redo().run()}>↷</button><i />
