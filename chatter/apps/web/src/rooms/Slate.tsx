...
export function Slate({ stories, me, onChanged }: { stories: Story[]; me?: User; onChanged: () => void }) {
  ...
  return <section className="view on newsroom-room slate-room">
    <header className="newsroom-hero"><div className="newsroom-hero-icon">▤</div><div><div className="newsroom-eyebrow">SLATE</div><h1>Story planning</h1><p>Create a pitch, set the angle, list sources, and assign the work.</p></div><button className="newsroom-button primary" onClick={async () => { if (!beforeLeave.current || await beforeLeave.current()) { setCreating(true); setNotice(undefined); } }}>＋ New story</button></header>
