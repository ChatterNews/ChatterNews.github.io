...
import {
  ...
  type MotionPackage, type MotionPreset, type Story, type User,
...
export function Stinger({ stories, me }: { stories: Story[]; me?: User }) {
  ...
  const reload = useCallback(async () => {
    ...
    setProject((current) => current && (!routeStoryId || current.storyId === routeStoryId) ? current : selectStoryWorkspace(ordered, { storyId: routeStoryId }));
  }, [store, routeStoryId]);
  ...
  if (!project) return (
    <section className="view on stinger-home">
      <header className="stinger-home-hero"><div><span className="stinger-eyebrow">STINGER · ON-SCREEN GRAPHICS</span><h1>Give the show its look.</h1><p>Build the headlines, name bars, openers, transitions, and end cards that appear over or between video.</p><div className="stinger-home-steps"><span><i>1</i> Pick a package</span><span><i>2</i> Choose a graphic</span><span><i>3</i> Change the words and motion</span><span><i>4</i> Send it to Showtime</span></div></div><div className="stinger-home-monitor"><i>ON AIR</i><b>CHATTER</b><strong>WEEKLY HEADLINES</strong><span>STINGER</span></div></header>
  ...
  return (
    <section className={`view on stinger-room ${project.creativeRecipe?.mode === 'GUIDED' ? 'stinger-guided' : 'stinger-freeform'}`}>
      ...
      <div className="stinger-topbar">
        ...
        <div className="stinger-history"><button className="stinger-icon-button" onClick={undoOnce} disabled={!undo.length} title="Undo"><span>↶</span><small>Undo</small></button><button className="stinger-icon-button" onClick={redoOnce} disabled={!redo.length} title="Redo"><span>↷</span><small>Redo</small></button></div>
        ...
        <div className="stinger-delivery-actions" role="group" aria-label="Check and export graphic">
          ...
          <button className="stinger-export" disabled={exporting !== undefined || !canRenderMotionVideo()} onClick={requestVideoExport}><Icon name="ic-bolt" /><span>{exporting === undefined ? 'Export this graphic' : `Rendering ${Math.round(exporting * 100)}%`}</span><small>WebM for Showtime</small></button>
        </div>
      </div>
