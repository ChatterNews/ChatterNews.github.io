...
export function Clubhouse({ stories, story: trackedStory, me, penName }: { stories: Story[]; story?: Story; me?: User; penName: string }) {
  ...
  return <section className="view on clubhouse-room">
    <header className="clubhouse-welcome"><div><span>CLUBHOUSE</span><h1>{penName.split(' ')[0]}'s desk</h1><p>{myTasks.length ? `${myTasks.length} active assignment${myTasks.length === 1 ? '' : 's'}. Closest deadline first.` : 'Your ideas belong here. Pick up a story or start something new.'}</p></div><button onClick={() => navigate('/slate')}>＋ New story</button></header>
